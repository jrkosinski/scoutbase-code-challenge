 ...
 * ------------------------
 * implements IDatabase as an in-memory data source for testing.
 *
...
class MockDB extends IDatabase {
    ...
    async /*bool*/ initialize(url) {
        return await exception.tryAsync(async () => {
            ...
            if (!client) {
                logger.warn('failed to connect to mockdb database');
            }
            else {
                const db = client.db();
                ...
                const movieDocs = [
                    ...
                    createMovie('Batman Begins', 2005, "PG-13", [
                        createPerson('Michael Caine', -1161388800, 'UK'),
                        createPerson('Christian Bale', 128736000, 'UK')
                    ], [
                        createPerson('Christopher Nolan', 18144000, 'UK')
                    ]),
    ...
    /*user*/ async getUser(username) {
        return await exception.tryAsync(async () => {
            ...
            if (username && username.length) {
                ...
                //find
                output = await users.findOne({username}, {});
